...
function toArray(x){
  ...
  else if(typeof x === 'undefined' || x === null) return [];
  return [x]
}
...
export default class TagSet {
  ...
  add(obj, tags){
    ...
    // add to tags
    for(var i=0; i<arr.length; i++) this.addTag(id, arr[i])
  ...
  addTag(id, tag){
    ...
    if(s) s.add(id);
    else this._tags[tag] = StringSet.singleton(id);
    return true
  };
  ...
  forEach(tag, callback){
    ...
    for(var i=0; i<ids.length; i++) callback(vals[i], ids[i])
  };
